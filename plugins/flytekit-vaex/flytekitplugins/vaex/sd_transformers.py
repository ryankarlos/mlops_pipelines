    ...
    def encode(
        ...
        return literals.StructuredDataset(
            ...
            metadata=StructuredDatasetMetadata(
                structured_dataset_type=StructuredDatasetType(structured_dataset_type)
            ),
        )
...
class ParquetToVaxDataFrameDecodingHandler(StructuredDatasetDecoder):
    ...
    def decode(
        ...
        if (
            ...
            columns = [
                c.name for c in current_task_metadata.structured_dataset_type.columns
            ]
...
class VaexDataFrameRenderer:
    ...
    def to_html(self, df: vaex.dataframe.DataFrameLocal) -> str:
        ...
        return pd.DataFrame(
            describe_df.transpose(), columns=describe_df.columns
        ).to_html(index=False)
...
StructuredDatasetTransformerEngine.register_renderer(
    vaex.dataframe.DataFrameLocal, VaexDataFrameRenderer()
)
