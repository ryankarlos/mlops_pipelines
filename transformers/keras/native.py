...
class KerasTypeTransformer(TypeTransformer, Generic[T]):
    def get_literal_type(self, t: Type[T]) -> LiteralType:
        return LiteralType(
            blob=_core_types.BlobType(
                format=self.KERAS_FORMAT,
    ...
    def to_literal(
        ...
        # save keras model to a folder in SavedModel format
        keras.models.save_model(python_val, local_path)
    ...
    def guess_python_type(self, literal_type: LiteralType) -> Type[T]:
        if (
            ...
            and literal_type.blob.format == self.KERAS_FORMAT
        ):
            return T
...
class KerasSequentialTransformer(KerasTypeTransformer[keras.Sequential]):
    KERAS_FORMAT = "KerasSequential"
